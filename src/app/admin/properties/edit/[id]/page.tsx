...
export default function EditPropertyPage() {
  ...
  const resetForm = useCallback((propertyData: any) => {
    form.reset({
      ...propertyData,
      features: Array.isArray(propertyData.features) ? propertyData.features : [],
      agentIds: Array.isArray(propertyData.agentIds) ? propertyData.agentIds.map(String) : [],
    });
  }, [form]);
  ...
  useEffect(() => {
    async function fetchData() {
      if (!id) return;
      setIsLoading(true);
      const [fetchedAgents, propertyData] = await Promise.all([
        getAgents(),
        getProperty(id)
      ]);
