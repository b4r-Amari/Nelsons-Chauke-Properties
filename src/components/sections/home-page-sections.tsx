...
export function CtaTabsSection() {
  ...
  const sellerOptions = [
    { id: 'book-valuation', title: "Book a Valuation", description: "Get an accurate, obligation-free valuation for your property from our expert agents.", buttonText: "Book Now", imageSrc: placeholders.freePropertyValuation.url, imageHint: placeholders.freePropertyValuation.hint, href: "/sell" },
    { id: 'selling-guides', title: "Selling Guides", description: "Navigate the selling process with confidence using our expert tips and guides.", buttonText: "Learn More", imageSrc: placeholders.sellingGuides.url, imageHint: placeholders.sellingGuides.hint, href: "/blog" },
  ];
    ...
    <div className="container mx-auto px-4">
      <Tabs defaultValue="buyers" className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-md mx-auto h-12">
          <TabsTrigger value="buyers" className="h-10">For Buyers</TabsTrigger>
          <TabsTrigger value="sellers" className="h-10">For Sellers</TabsTrigger>
        </TabsList>
        <TabsContent value="buyers">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
            {buyerOptions.map(option => (
              <CtaCard key={option.id} {...option} />
            ))}
          </div>
        </TabsContent>
...
// Sub-component for CTA cards
function CtaCard({ title, description, buttonText, imageSrc, imageHint, href = "#" }: { title: string, description: string, buttonText: string, imageSrc: string, imageHint: string, href?: string }) {
  return (
    <div className="relative rounded-lg overflow-hidden text-white group">
      <Image
        src={imageSrc}
        alt={imageHint}
        fill
