...
export function MortgageBondCalculator() {
  const [result, setResult] = useState({ totalPayment: 0, totalInterest: 0 });
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    if (loanAmount <= 0 || monthlyInterestRate <= 0 || numberOfPayments <= 0) {
        setResult({ totalPayment: 0, totalInterest: 0 });
        return;
    }
    ...
    setResult({ totalPayment, totalInterest });
  };
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        <CardTitle className="font-headline text-2xl">Mortgage Bond Total Cost Calculator</CardTitle>
        <CardDescription>Calculate the total amount you will pay over the life of your bond, including all interest.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div className="bg-muted/50 p-8 rounded-lg space-y-6 flex flex-col justify-center">
            <h3 className="text-xl font-bold font-headline text-center">Your Bond Cost Results</h3>
            <div className="text-center">
              <p className="text-sm text-muted-foreground">Total Repayment Over Loan Term</p>
              <p className="text-4xl font-bold text-brand-bright">{formatCurrency(result.totalPayment)}</p>
            </div>
            <div className="space-y-4 pt-4 border-t">
              <div className="flex justify-between items-center">
                <span className="font-semibold">Principal Loan Amount</span>
                <span className="font-mono text-lg">{formatCurrency(form.getValues().loanAmount)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-semibold text-destructive">Total Interest Paid</span>
                <span className="font-mono text-lg text-destructive">{formatCurrency(result.totalInterest)}</span>
              </div>
            </div>
          </div>
        </div>
